...
export async function GET(request: Request): Promise<Response> {
  ...
  return textResponse(
    [
      ...
      `Find YouTube channels by name/topic: GET ${origin}/api/channel-search?q=QUERY`,
      `Channel search status: ${origin}/channel-searches/JOB_ID.json`,
      `Channel status: ${origin}/channels/CHANNEL_JOB_ID.json`,
      "Channel jobs enumerate all public uploads in batches and report workflow progress, transcript coverage, grouped failure reasons, and ETA.",
      "",
